...
class Tree {
  public:
    ...
    int64_t range_sum(int64_t left, int64_t right) {
		...
		Node* left_node = lookup_nearest(
			left, 
			[](auto&& key_1, auto&& key_2) { return key_1 < key_2; });
		left_node->left = nullptr;

		Node* right_node = lookup_nearest(
			left,
			[](auto&& key_1, auto&& key_2) { return key_1 > key_2; });
...
private:
	...
	// Look up the given key in the tree, returning the
	// the node with the requested key or ....
	template<typename Func>
	Node* lookup_nearest(int64_t key, Func compare) {
		Node* node = root;
		Node* node_last = nullptr;
		while (node) {
			node_last = node;
			...
			if (compare(key, node->key))
				...
				node = node->right; // TODO: switch l/r for '>'
		}
		if (node_last)
			splay(node_last);
		return node_last;
